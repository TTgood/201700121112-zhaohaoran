...
SRC_TRAIN_DATA = "samples/question2_all_id.txt"          # 源语言输入文件。
TRG_TRAIN_DATA = "samples/answer2_all_id.txt"          # 目标语言输入文件。
CHECKPOINT_PATH = "./atten/attention_ckpt"   # checkpoint保存路径。  

HIDDEN_SIZE = 1024                     # LSTM的隐藏层规模。
DECODER_LAYERS = 2                     # 解码器中LSTM结构的层数。这个例子中编码器固定使用单层的双向LSTM。
SRC_VOCAB_SIZE = 70000                # 源语言词汇表大小。
TRG_VOCAB_SIZE = 70000                 # 目标语言词汇表大小。
BATCH_SIZE = 250                       # 训练数据batch的大小。
NUM_EPOCH = 200                          # 使用训练数据的轮数。
KEEP_PROB = 0.8                        # 节点不被dropout的概率。
MAX_GRAD_NORM = 5                      # 用于控制梯度膨胀的梯度大小上限。
SHARE_EMB_AND_SOFTMAX = True           # 在Softmax层和词向量层之间共享参数。

MAX_LEN = 20   # 限定句子的最大单词数量。
...
class NMTModel(object):
    ...
    def forward(self, src_input, src_size, trg_input, trg_label, trg_size):
        ...
        with tf.variable_scope("decoder"):
            ...
            # memory_sequence_length代表batch中每个句子的长度，
